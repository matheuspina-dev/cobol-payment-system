       ...
       FILE-CONTROL.
           ...
           SELECT UPD-ACC-FILE ASSIGN TO 'output/ACCOUNTS-UPDATED.TXT'
               ORGANIZATION IS LINE SEQUENTIAL
      ...
      * --- IN-MEMORY DATA STRUCTURES ---
       ...
       0000-MAIN-PROCEDURE.
           ...
           PERFORM 4000-PROCESS-TRANSACTIONS
               UNTIL END-OF-FILE
           PERFORM 8500-WRITE-UPDATED-ACCOUNTS
       ...
       1000-INIT-FILES.
           ...
           OPEN OUTPUT OUT-FILE ERR-FILE REP-FILE UPD-ACC-FILE
           ...
           PERFORM UNTIL END-OF-FILE
               READ CONFIG-FILE
                   ...
                   NOT AT END
                       ...
                       EVALUATE FUNCTION TRIM(WS-CONF-KEY)
                           WHEN "MAX_DEBIT"
                               COMPUTE WS-MAX-DEBIT =
                                   FUNCTION NUMVAL(WS-CONF-VAL)
                           WHEN "ALLOW_CREDIT"
                               MOVE FUNCTION TRIM(WS-CONF-VAL)
                                   TO WS-ALLOW-CREDIT
                       END-EVALUATE
               END-READ
           END-PERFORM.
       ...
       5000-VALIDATE-AND-APPLY.
           UNSTRING TRANS-REC DELIMITED BY ","
           ...
           IF TXN-APPROVED
               IF T-TYPE NOT = "DEBIT" AND T-TYPE NOT = "CREDIT"
           ...
           IF TXN-APPROVED
               IF T-TYPE = "CREDIT" AND WS-ALLOW-CREDIT NOT = 'Y'
       ...
       6000-WRITE-APPROVAL.
           ...
           STRING "APPROVED: " T-ID " NEW BALANCE: "
               TAB-ACC-BAL(ACC-IDX)
       ...
       8500-WRITE-UPDATED-ACCOUNTS.
           PERFORM VARYING ACC-IDX FROM 1 BY 1
               UNTIL ACC-IDX > WS-ACC-COUNT
               MOVE SPACES TO UPD-ACC-REC
               STRING TAB-ACC-ID(ACC-IDX) ","
                   TAB-ACC-BAL(ACC-IDX)
                   DELIMITED BY SIZE INTO UPD-ACC-REC
               WRITE UPD-ACC-REC
               IF NOT UPD-OK
                   DISPLAY "I/O ERROR ON UPDATED ACCOUNTS WRITE."
               END-IF
           END-PERFORM.

       9000-CLEANUP.
           CLOSE CONFIG-FILE ACC-FILE TRANS-FILE
                 OUT-FILE ERR-FILE REP-FILE UPD-ACC-FILE.
